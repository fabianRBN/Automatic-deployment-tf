...
resource "aws_security_group" "db" {
  name        = "sgp-db-${var.environment}"
  description = "Security group for MongoDB server in ${var.environment}"
  vpc_id      = var.vpc_id

  # Permitir tráfico MongoDB (puerto 27017)
  ingress {
    description      = "HTTPS"
    from_port        = 443
    to_port          = 443
